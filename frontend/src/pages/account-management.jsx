...
import { Activity, Check, Loader2, Mail, MailPlus, RefreshCw, Users, UserPlus, Shield, User, CalendarDays, Trash2, ChevronDown } from "lucide-react"
                  ...
                  return (
                    <div key={user.id} className="p-4 space-y-3 transition-colors hover:bg-slate-50/50">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className={cn(
                          "inline-flex items-center rounded-full px-2.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wide border shrink-0",
                          user.status === 'active'
                  ...
                  filteredUsers.map((user) => {
                    ...
                    return (
                      <tr key={user.id} className="hover:bg-slate-50/40 transition-colors group">
                        ...
                        <td className="px-6 py-4">
                          <span className={cn(
                            "inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-extrabold uppercase tracking-wide border",
                            user.status === 'active'
                              ? "bg-emerald-50 text-emerald-700 border-emerald-100"
                              : "bg-amber-50 text-amber-700 border-amber-100"
                          )}>
                            <span className={cn("size-1.5 rounded-full mr-1.5 shrink-0", user.status === 'active' ? "bg-emerald-500" : "bg-amber-500")} />
                            {user.status === 'active' ? 'ACTIF' : 'INVITÉ'}
                          </span>
                        </td>
                        <td className="px-6 py-4">
